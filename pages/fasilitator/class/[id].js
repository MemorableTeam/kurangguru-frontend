...
const classDetail = () => {
  ...
  return (
    <>
          ...
          <Col md={2} lg={3} xl={3}>
            ...
            <Card className='border-radius-10 p-3 mt-3'>
              ...
              {member?.data?.status !== 400 && member?.map(item => {
                return (<>
                  <div className='w-100 d-flex justify-content-start align-items-center' onClick={() => { setForm({ user_id: item?.user_id, token: auth?.user?.token, class_id: id }); setShowModal(true) }}>
                    <input type="checkbox" className="px-2 text-center mt-1 ms-2" value={item?.id} />
      ...
      <ModalMember
        show={showModal}
        onHide={() => { setShowModal(false); setForm(null) }}
        form={form}
        route={{ icon: '../../icon', img: '../../images' }}
      />
    </>
  )
}
